...
def _declare_item(
    ...
    CBcode = cbcode if cbcode else f"CB{self.__random_n_digits(14)}"
    lpcode = lpcode if lpcode else f"LP{self.__random_n_digits(14)}"

    feature = {
        "linehaulResCode": "TRUNK_30145010",
        "declarationRole": f"warehouse{'' if self.product['source_code']=='DISTRIBUTOR_30875031' else 'nooit'}",
        "laneCode": "L_AE_ECONOMY_NLAIR_RM",
        "distributorResCode": self.product['source_code']
    }
    ...
    if from_file:
        with open('customs_declare_item.json') as json_file:
            obj_content = json.load(json_file)
            if not isinstance(obj_content['feature'], str):
                obj_content['feature'] = json.dumps(obj_content['feature'])
