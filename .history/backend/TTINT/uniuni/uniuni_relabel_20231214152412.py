...
def connect_uni_ca(barcode, alternative, yyyymmdd, hhmmss):
...
def run(param):
    out = []
    list = get_list(param['txt'])
    yyyymmdd, hhmmss= param['utc'].split('T')
    for barcode, alternative, in list:
        res =  connect_uni_ca(barcode, alternative, yyyymmdd=yyyymmdd, hhmmss=hhmmss)
        out.append( f"{barcode}={alternative} : {res}" )
    
    
    
    return

param = {'utc': '2023-12-14T13:09:51', 'txt': '\nSPRCA00000083358 = 1052071003453605\nSPRCA00000083358 = 1052071003453605'}
